...
class EntityDataPreferencesResource extends AbstractResourceListener implements EventManagerAwareInterface
{
       ...
       try
        {
            ...
            $entityData = $this->_dataModel->getById($dataId);
            $entityDataId = $entityData->Id;
            if( !isset($entityDataId) || empty($entityDataId) ){
                throw new \InvalidArgumentException("No entity data with id = $dataId",1);
            }
            
            $this->getEventManager()->trigger('entity_data_preferences.create.pre', $this, array());
            ...
            $entityDataPreference = new EntityDataPreferencesEntity();
            $entityDataPreference->hydrate($data);
            $entityDataPreference->Status = 1;
            $entityDataPreference->DataId = $dataId;
            $entityDataPreference->CreationDate = $creationDate;
            ...
            if($exists){
                $this->_model->update($entityDataPreference->getArrayCopy(),array('Id'=>$exists->Id));
            }else{
                $inserted = $this->_model->insert($entityDataPreference->getArrayCopy());
                if($inserted){
                    $entityDataPreference->Id = $this->_model->getLastInsertValue();
                }
            }
            
            $this->getEventManager()->trigger('entity_data_preferences.create.post', $this, array('entity' => $entityDataPreference, 'routeParams' => $this->getEvent()->getRouteMatch()->getParams()) );

            return $entityDataPreference->getExtendedData();
        }
    ...
    public function delete($id)
    {
        try
        {
            ...
            $entityData = $this->_dataModel->getById($dataId);
            $entityDataId = $entityData->Id;
            if( !isset($entityDataId) || empty($entityDataId) || $entityData->Status == 99 )
            {
                throw new \Exception("No entity data with id = $dataId", 1);
            }
    ...
    public function fetchAll($params = [])
    {
        try
        {
            ...
            if( $dataId == '*' )
            {
                ...
                $result = array('entity_data_preferences' => $preferences);
            ...
            else
            {
                ...
                $result = array('entity_data_preferences' => $extendedCode);
                // $collection = new \ArrayIterator($result);
                $collection = new \Zend\Paginator\Adapter\ArrayAdapter($result);
            }

            // return new EntityDataPreferencesCollection(new \Zend\Paginator\Adapter\Iterator($collection));
            return new EntityDataPreferencesCollection($collection);
        }
        catch(\Exception $e)
        {
            return new ApiProblem(417, $e->getMessage());
