...
class EntityProfilePreferencesResource extends AbstractResourceListener implements EventManagerAwareInterface
{
    ...
    public function create($data)
    {
        try
        {
            ...
            $entityProfile = $this->_profilesModel->getById($profileId);
            $entityProfileId = $entityProfile->Id;
            if( !isset($entityProfileId) || empty($entityProfileId) || $entityProfile->Status == 99 )
            {
                throw new \Exception("No entity profile with id = $profileId",1);
            }

            $this->getEventManager()->trigger('entity_profile_preferences.create.pre', $this, array());
            ...
            $entityProfilePreference = new EntityProfilePreferencesEntity();
            $entityProfilePreference->hydrate($data);
            $entityProfilePreference->Status = 1;
            $entityProfilePreference->ProfileId = $profileId;
            $entityProfilePreference->CreationDate = $creationDate;
            ...
            if( $exists )
            {
                $this->_model->update($entityProfilePreference->getArrayCopy(),array('Id'=>$exists->Id));
            }
            else
            {
                $inserted = $this->_model->insert($entityProfilePreference->getArrayCopy());
                if( $inserted )
                {
                    $entityProfilePreference->Id = $this->_model->getLastInsertValue();
                }
            }

            $this->getEventManager()->trigger('entity_profile_preferences.create.post', $this, array('entity' => $entityProfilePreference, 'routeParams' => $this->getEvent()->getRouteMatch()->getParams()) );

            return $entityProfilePreference->getExtendedData();
        }
    ...
    public function delete($id)
    {
        try
        {
            ...
            $this->getEventManager()->trigger('entity_profile_preferences.delete.pre', $this, array());

            $result = $this->_model->delete(array('Id' => $id,'ProfileId' => $profileId));

            $this->getEventManager()->trigger('entity_profile_preferences.delete.post', $this, array('id' => $id, 'adapter' => $this->_model->getAdapter(), 'routeParams' => $this->getEvent()->getRouteMatch()->getParams()) );
    ...
    public function fetchAll($params = array())
    {
        try
        {
            ...
            if( $profileId == '*' )
            {
                ...
                $result = array('entity_profile_preferences' => $preferences);
            ...
            else
            {
                ...
                $result = array('entity_profile_preferences' => $extendedRecord);
                // $collection = new \ArrayIterator($result);
                $collection = new \Zend\Paginator\Adapter\ArrayAdapter($result);
            }

            // return new EntityProfilePreferencesCollection(new \Zend\Paginator\Adapter\Iterator($collection));
            return new EntityProfilePreferencesCollection($collection);
        }
        catch(\Exception $e)
        {
            return new ApiProblem(417, $e->getMessage());
    ...
    public function patch($id, $data)
    {
        try
        {
            ...
            $this->getEventManager()->trigger('entity_profile_preferences.patch.pre', $this, array());
            ...
            $this->getEventManager()->trigger('entity_profile_preferences.patch.post', $this, array('id' => $id, 'data' => $data, 'routeParams' => $this->getEvent()->getRouteMatch()->getParams()) );
