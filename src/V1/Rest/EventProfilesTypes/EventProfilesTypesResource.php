...
class EventProfilesTypesResource extends AbstractResourceListener implements EventManagerAwareInterface
{
    ...
    public function create($data)
    {
        try
        {
            ...
            /*$typeId = $this->getEvent()->getRouteParam('event_profile_types_id');
            if( !isset($typeId) || empty($typeId) )
            {
                throw new \InvalidArgumentException("Please specify an event profile type!", 1);
            }
            ...
            $this->getEventManager()->trigger('event_profiles_types.create.pre', $this, array());
            ...
            if( !$inserted )
            {
                throw new \Exception("Could not associate event profiles types", 1);
            }
            $entity->Id = $this->_model->getLastInsertValue();

            $this->getEventManager()->trigger('event_profiles_types.create.post', $this, array());
    ...
    public function delete($id)
    {
        try
        {
            ...
            $this->getEventManager()->trigger('event_profiles_types.delete.pre', $this, array());
            ...
            $this->getEventManager()->trigger('event_profiles_types.delete.post', $this, array('id' => $id, 'adapter' => $this->_model->getAdapter(), 'routeParams' => $this->getEvent()->getRouteMatch()->getParams()) );
    ...
    public function fetch($id)
    {
        try
        {
            ...
            if( $profileId == '*' )
            {
                $eventProfilesTypes = $this->_model->getAllProfilesByTypeId($id);
                return new EventProfilesTypesCollection(new \Zend\Paginator\Adapter\Iterator($eventProfilesTypes));
            }
            else
            {
                if( $id == '*' )
                ...
                $eventProfileType = $this->_model->getByProfileIdAndTypeId($profileId, $id);

                if( !$eventProfileType || $eventProfileType->Status == 99 )
                {
                    throw new \InvalidArgumentException("Error Processing Request", 1);
                }

                return ($eventProfileType->Id) ? $eventProfileType : false;
            }
        }
        catch(\InvalidArgumentException $e)
        {
    ...
    public function fetchAll($params = [])
    {
        try
        {
            ...
            return new EventProfilesTypesCollection(new \Zend\Paginator\Adapter\Iterator($collection));
        }
        catch(\InvalidArgumentException $e)
        {
            return new ApiProblem(400, $e->getMessage());
