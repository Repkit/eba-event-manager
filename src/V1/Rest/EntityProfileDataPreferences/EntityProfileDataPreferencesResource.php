...
class EntityProfileDataPreferencesResource extends AbstractResourceListener implements EventManagerAwareInterface
{
    ...
    public function create($data)
    {
        try
        {
            $profileDataId = $this->getEvent()->getRouteParam('entity_profile_data_id');
            ...
            $entityProfileId = $profileData->Id;
            if( !isset($entityProfileId) || empty($entityProfileId) || $profileData->Status == 99 )
            {
                throw new \Exception("No entity profile data with id = $profileDataId",1);
            }

            $this->getEventManager()->trigger('entity_profile_data_preferences.create.pre', $this, array());
            ...
            $profileDataPreference = new EntityProfileDataPreferencesEntity();
            ...
            $this->getEventManager()->trigger('entity_profile_data_preferences.create.post', $this, array('entity' => $profileDataPreference, 'routeParams' => $this->getEvent()->getRouteMatch()->getParams()) );
    ...
    public function delete($id)
    {
        try
        {
            $dataId = $this->getEvent()->getRouteParam('entity_profile_data_id');
            ...
            {
                throw new \Exception("No entity profile data with id = $dataId", 1);
            }
    ...
    public function fetchAll($params = [])
    {
        try
        {
            ...
            if( $dataId == '*' )
            {
                ...
                $result = array('entity_profile_data_preferences' => $preferences);
            ...
            else
            {
                ...
                $result = array('entity_profile_data_preferences' => $extendedCode);
                // $collection = new \ArrayIterator($result);
                $collection = new \Zend\Paginator\Adapter\ArrayAdapter($result);
            }

            // return new EntityProfileDataPreferencesCollection(new \Zend\Paginator\Adapter\Iterator($collection));
            return new EntityProfileDataPreferencesCollection($collection);
