...
class EntityProfileDataModel extends Model
{
    protected $EntityClass  = 'MicroIceEventManager\V1\Rest\EntityProfileData\EntityProfileDataEntity';
    protected $TableName    = 'entity_profile_data';
    ...
    public function fetchAllByProfileIdAndTypeId($ProfileId, $TypeId, $Options = array(), $Filter = null)
    {
        ...
        $select->join('entity_profiles_data'
            , new Expression($this->TableName . '.Id = entity_profiles_data.DataId AND entity_profiles_data.Status = 1')
            , array() // do not select anything
            , Select::JOIN_INNER
        );
        $select->join('entity_profile_translations'
                , new Expression('entity_profile_translations.Id = entity_profiles_data.ProfileId')
                , array() // do not select anything
                , Select::JOIN_INNER
            );
        ...
        $select->where->equalTo('entity_profile_translations.Id',$ProfileId);
