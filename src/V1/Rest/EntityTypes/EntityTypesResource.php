...
class EntityTypesResource extends AbstractResourceListener implements EventManagerAwareInterface
{
    ...
    public function create($data)
    {
        try
        {
            $this->getEventManager()->trigger('entity_types.create.pre', $this, array());

            $id = $this->getEvent()->getRouteParam('entity_type_id');
            ...
            if($default)
            {
                ...
                if(!empty($rdata['Name'])){
                    ...
                    if(!empty($texists)){
                        throw new \InvalidArgumentException("An entity type with same name already exists!", 1);
                    }
                }
                ...
                if(!$inserted){
                    throw new \Exception("Could not create entity type", 1);
                }
            ...
            if(!$inserted){
                throw new \Exception("Could not create entity type translation", 2);
            }
            ...
            $this->getEventManager()->trigger('entity_types.create.post', $this, array('entity' => $entity, 'routeParams' => $this->getEvent()->getRouteMatch()->getParams()) );
    ...
    public function delete($id)
    {
        try
        {
            ...
            if(!$type || $type->Status != 1){
                throw new \Exception("Please specify an active entity type!", 1);
            }
            unset($type);*/
            
            $this->getEventManager()->trigger('entity_types.delete.pre', $this, array());
            ...
            $this->getEventManager()->trigger('entity_types.delete.post', $this, array('id' => $id, 'adapter' => $this->_model->getAdapter(), 'routeParams' => $this->getEvent()->getRouteMatch()->getParams()) );
    ...
    public function fetch($id)
    {
        try
        {
            ...
            {
                $storage = array();
                if(!empty($this->_settings['entity_type'])){
                    if(!empty($this->_settings['entity_type']['storage'])){
                        if(!empty($this->_settings['entity_type']['storage']['joins'])){
                            $storage = $this->_settings['entity_type']['storage'];
                        }
                    }
                }

                /*$where = new \Zend\Db\Sql\Where();
                $where->equalTo('entity_types.Id',$id);*/
            ...
            return new EntityTypesCollection(new \Zend\Paginator\Adapter\Iterator($entities));
        ...
        try
        {
            ...
            if(count($params))
            {
                ...
                if($fetchall)
                {
                    ...
                    // $postwhere->isNull('entity_type_translations.Language');
            ...
            {
                // exclude 99
                $entities = $this->_model->fetchAll(array('where'=>array('entity_types.Status' => array(0,1))));    
            }
    ...
    public function patch($id, $data)
    {
        try
        {
            ...
            $this->getEventManager()->trigger('entity_types.update.pre', $this, array());
            ...
            $this->getEventManager()->trigger('entity_types.update.post', $this, array('id' => $id, 'data' => $data, 'routeParams' => $this->getEvent()->getRouteMatch()->getParams()) );
