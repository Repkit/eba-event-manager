...
use MicroIceEventManager\V1\Rest\EventProfilesTypes\EventProfilesTypesEntity;
...
class EventProfileDetailsResource extends AbstractResourceListener implements EventManagerAwareInterface
{
    ...
    public function create($data)
    {
        try
        {
            ...
            {
                throw new \InvalidArgumentException("Please specify an event profile type for the event!", 1);
            }
            ...
            // validate event profile translation
            $translationId = $this->getEvent()->getRouteParam('event_profile_translation_id');
            if( !isset($translationId) || empty($translationId) )
            {
                throw new \InvalidArgumentException("Please specify an event profile translation for the event!", 1);
            }

            $profileTranslation = $this->_profileTranslation->getById($translationId);
            if( !$profileTranslation || $profileTranslation->Status != 1 )
            {
                throw new \InvalidArgumentException("Please specify a valid event profile translation!", 1);
            }
            
            $this->getEventManager()->trigger('event_profile_details.create.pre', $this, array());
            ...
            foreach ($fieldcfg as $key => $field) 
            {
                ...
                // create entity
                $profileField = new EventProfileDetailsEntity();
            ...
            $profilesTypes = new EventProfilesTypesEntity();
            ...
            if(!isset($assigned) || empty($assigned)){
                throw new \Exception("Could not assign data to Event Profiles Types!", 1);
            }

            $this->getEventManager()->trigger('event_profile_details.create.post', $this, array('entity' => $data, 'routeParams' => $this->getEvent()->getRouteMatch()->getParams()) );
    ...
    public function fetch($id)
    {
        try
        {
            ...
            return new EventProfileDetailsCollection(new \Zend\Paginator\Adapter\Iterator($fields));
        }
        catch(\InvalidArgumentException $e)
        {
            return new ApiProblem(400, $e->getMessage());
    ...
    public function fetchAll($params = array())
    {
        try
        {
            ...
            // return new EventProfileDetailsCollection(new \Zend\Paginator\Adapter\Iterator($data));
            return new EventProfileDetailsCollection($data);
        }
        catch(\InvalidArgumentException $e)
        {
            return new ApiProblem(400, $e->getMessage());
    ...
    public function patch($id, $data)
    {
        try
        {
            ...
            $this->getEventManager()->trigger('event_profile_details.patch.pre', $this, array());
            ...
            foreach ($fields as $key => $field)
            {
                ...
                // create entity
                $udetails = new EventProfileDetailsEntity();
            ...
            $this->getEventManager()->trigger('event_profile_details.patch.post', $this, array('id' => $id, 'data' => $data, 'routeParams' => $this->getEvent()->getRouteMatch()->getParams()) );
