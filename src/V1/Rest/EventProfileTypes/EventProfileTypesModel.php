...
class EventProfileTypesModel extends Model
{
	protected $EntityClass 	= 'MicroIceEventManager\V1\Rest\EventProfileTypes\EventProfileTypesEntity';
	protected $TableName 	= 'event_profile_types';
    ...
    public function getById($Id)
    {
        ...
        $select->join('event_profile_type_translations'
            , new Expression($this->TableName . '.Id = event_profile_type_translations.TypeId')
        ...
        $select->where->isNull('event_profile_type_translations.Language');
    ...
    public function getTranslationsById($Id)
    {
        ...
        $select->from($this->TableName);
        $select->join('event_profile_type_translations'
            , new Expression($this->TableName . '.Id = event_profile_type_translations.TypeId AND event_profile_type_translations.Status = 1')
            , array('TranslationId'=>'Id', 'Language', 'Name')
            , Select::JOIN_INNER
        );
        // $select->where->notEqualTo($this->TableName . '.Status', 99);
    ...
    public function getAllProfileTypesByLanguage($Language = null)
    {
        ...
        if(null == $Language){
            ...
            $select->where->equalTo('event_profile_type_translations.Language', $Language);
        }
