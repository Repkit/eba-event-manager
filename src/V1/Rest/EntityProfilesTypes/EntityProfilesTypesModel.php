...
class EntityProfilesTypesModel extends Model
{
	protected $EntityClass 	= 'MicroIceEventManager\V1\Rest\EntityProfilesTypes\EntityProfilesTypesEntity';
	protected $TableName 	= 'entity_profiles_types';
    ...
    public function getByProfileIdAndTypeId($ProfileId, $TypeId, array $where = array())
    {
        ...
        $select->join('entity_profile_types'
            , new Expression($this->TableName . '.TypeId = entity_profile_types.Id')
            , array()
            , Select::JOIN_INNER
        );
        $select->join('entity_profile_type_translations'
            , new Expression('entity_profile_types.Id = entity_profile_type_translations.TypeId AND entity_profile_type_translations.Language IS NULL')
            , array('Name')
            , Select::JOIN_INNER
        );
    ...
    public function getAllTypesByProfileIdAndLanguage($ProfileId, $Language = null, array $where = array())
    {
        ...
        $select->join('entity_profile_type_translations'
            , new Expression('entity_profile_types.Id = entity_profile_type_translations.TypeId AND entity_profile_type_translations.Language ' . $langCond)
            , array('Name')
            , Select::JOIN_INNER
        );
