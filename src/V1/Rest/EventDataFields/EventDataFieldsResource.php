...
class EventDataFieldsResource extends AbstractResourceListener implements EventManagerAwareInterface
{
    ...
    public function create($data)
    {
        ...
        try
        {
            ...
            if(!isset($dataId) || empty($dataId)){
                throw new \InvalidArgumentException("Please specify an event data for the event!", 1);
            }

            $dbdata = $this->_data->getById($dataId);

            if(!$dbdata || $dbdata->Status != 1){
                throw new \InvalidArgumentException("Please specify a valid event data!", 1);
            }
            
            $this->getEventManager()->trigger('event_data_fields.create.pre', $this, array());
            ...
            $this->getEventManager()->trigger('event_data_fields.create.post', $this, array('entity' => $data, 'routeParams' => $this->getEvent()->getRouteMatch()->getParams()) );
    ...
    public function fetchAll($params = array())
    {
        try
        {
            ...
            return new EventDataFieldsCollection(new \Zend\Paginator\Adapter\Iterator($fields));
        }
        catch(\InvalidArgumentException $e)
        {
            return new ApiProblem(400, $e->getMessage());
