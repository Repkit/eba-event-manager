...
class EntityProfileDataFieldsResource extends AbstractResourceListener implements EventManagerAwareInterface
{
    ...
    public function create($data)
    {
        try
        {
            // validate entity profile data
            $profileDataId = $this->getEvent()->getRouteParam('entity_profile_data_id');
            if( !isset($profileDataId) || empty($profileDataId) )
            {
                throw new \InvalidArgumentException("Please specify an entity profile data for the entity!", 1);
            }

            $dbdata = $this->_profileData->getById($profileDataId);
            if( !$dbdata || $dbdata->Status != 1 )
            {
                throw new \InvalidArgumentException("Please specify a valid entity profile data!", 1);
            }

            $this->getEventManager()->trigger('entity_profile_data_fields.create.pre', $this, array());
            ...
            $this->getEventManager()->trigger('entity_profile_data_fields.create.post', $this, array('entity' => $data, 'routeParams' => $this->getEvent()->getRouteMatch()->getParams()) );
    ...
    public function fetchAll($params = [])
    {
        try
        {
            ...
            return new EntityProfileDataFieldsCollection(new \Zend\Paginator\Adapter\Iterator($fields));
        }
        catch(\InvalidArgumentException $e)
        {
            return new ApiProblem(400, $e->getMessage());
