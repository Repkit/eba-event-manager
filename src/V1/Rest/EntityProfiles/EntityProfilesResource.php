...
class EntityProfilesResource extends AbstractResourceListener implements EventManagerAwareInterface
{
    ...
    public function create($data)
    {
        try
        {
            $id = $this->getEvent()->getRouteParam('entity_profile_id');
            ...
            $this->getEventManager()->trigger('entity_profiles.create.pre', $this, array());
            ...
            if( $default )
            {
                ...
                if( !empty($rdata['Name']) )
                {
                    ...
                    if( !empty($texists) )
                    {
                        throw new \InvalidArgumentException("An entity profile with same name already exists!", 1);
                    }
                }
                ...
                if( !$inserted )
                {
                    throw new \Exception("Could not create entity profile", 1);
                }
            ...
            if( !$inserted )
            {
                throw new \Exception("Could not create entity profile translation", 2);
            }
            ...
            $this->getEventManager()->trigger('entity_profiles.create.post', $this, array('entity' => $entity, 'routeParams' => $this->getEvent()->getRouteMatch()->getParams()));
    ...
    public function delete($id)
    {
        try
        {
            $this->getEventManager()->trigger('entity_profiles.delete.pre', $this, array());
            ...
            $this->getEventManager()->trigger('entity_profiles.delete.post', $this, array('id' => $id, 'adapter' => $this->_model->getAdapter(), 'routeParams' => $this->getEvent()->getRouteMatch()->getParams()));
    ...
    public function fetch($id)
    {
        try
        {
            ...
            if( $lang == '*')
            {
                $entityProfiles = $this->_model->getTranslationsById($id);
            }
            else
            {
                if( !empty($lang) )
                {
                    if( $id == '*' )
                    {
                        $entityProfiles = $this->_model->getAllProfilesByLanguage($lang);
                    }
                    else
                    {
                        $entityProfile = $this->_model->getByIdAndLanguage($id, $lang);
                        if( !$entityProfile )
                        {
                            // if no translation return default
                            $entityProfile = $this->_model->getById($id);
                        }
                    }
                }
                else
                ...
                {
                    if( !isset($entityProfiles) )
                    {
                        throw new \InvalidArgumentException("Error Processing Request", 1);
                    }
                }

                if( !isset($entityProfiles) && isset($entityProfile) )
                {
                    $entityProfiles = array($entityProfile);
                    $entityProfiles = new \ArrayIterator($entityProfiles);
                }
            }

            $collection = array();
            foreach ($entityProfiles as $key => $entityProfile)
            {
                if( !$entityProfile || $entityProfile->Status == 99 )
                {
                    throw new \InvalidArgumentException("Error Processing Request", 1);
                ...
                $tid = $entityProfile->TranslationId;
                if( empty($tid) )
                {
                    continue;
                }
                ...
                $profilelang = $entityProfile->Language;
                if( $profilelang == null )
                {
                ...
                foreach ($types as $k => $type)
                {
                    ...
                    //data types
                    $profileDataTypesDb = $this->_profileTypesDataTypes->getAllEntityProfileDataTypesByEntityProfileTypeId($typeId);
                    ...
                    foreach ($profileDataTypesDb as $j => $dataType)
                    {
                        ...
                        // get extra fields for all entity profile data
                ...
                $entityProfile->Types = $profileTypes;
                $collection[] = $entityProfile;
            }

            // $entityProfiles = new \ArrayIterator($collection);
            $entityProfiles = new \Zend\Paginator\Adapter\ArrayAdapter($collection);
            // return new EntityProfilesCollection(new \Zend\Paginator\Adapter\Iterator($entityProfiles));
            return new EntityProfilesCollection($entityProfiles);
        }
        catch(\InvalidArgumentException $e)
        {
            return new ApiProblem(400, $e->getMessage());
    ...
    public function fetchAll($params = array())
    {
        try
        {
            $entityProfiles = $this->_model->fetchAll();
    ...
    public function patch($id, $data)
    {
        try
        {
            ...
            $this->getEventManager()->trigger('entity_profiles.patch.pre', $this, array());
            ...
            {
                // update translated entity profile
            ...
            $this->getEventManager()->trigger('entity_profiles.patch.post', $this, array('id' => $id, 'data' => $data, 'routeParams' => $this->getEvent()->getRouteMatch()->getParams()));
    ...
    public function update($id, $data)
    {
        try
        {
            ...
            $this->getEventManager()->trigger('entity_profiles.update.pre', $this, array());
            ...
            unset($updateData['Timestamp']);
            $this->_model->update($updateData, array('Id' => $id));
            $this->getEventManager()->trigger('entity_profiles.update.post', $this, array('id' => $id, 'data' => $data, 'routeParams' => $this->getEvent()->getRouteMatch()->getParams()));
            $connection->commit();
